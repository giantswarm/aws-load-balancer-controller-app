...
func newNginxIngressApp() (*application.Application, error) {
	org := state.GetCluster().Organization
	helloWorldApp := application.New(fmt.Sprintf("%s-ingress-nginx", state.GetCluster().Name), "ingress-nginx").
		...
		MustWithValuesFile("./test_data/nginx_values.yaml", &application.TemplateValues{})
...
func serviceHasFinalizer(serviceNamespace, serviceName, finalizer string) (bool, error) {
	...
	logger.Log("Checking if nginx Service has the aws-load-balancer-controller finalizer")
	service := &v1.Service{}
